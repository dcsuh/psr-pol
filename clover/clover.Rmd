...
Read data  
```{r, message = F}
mammal <- read_csv("/Users/dcsuh/Desktop/clover/CLOVER_0.1_MammalViruses_AssociationsFlatFile.csv")
columns <- read_csv("/Users/dcsuh/Desktop/clover/CLOVER_ColumnDescriptions.csv")
```
...
```{r}
gmpd <- read_csv("/Users/dcsuh/Desktop/GMPD_datafiles/GMPD_main.csv")
```

More data
```{r, message=F}
viruses <- read_csv("/Users/dcsuh/Desktop/clover/CLOVER_1.0_Viruses_AssociationsFlatFile.csv")
helminths <- read_csv("/Users/dcsuh/Desktop/clover/CLOVER_1.0_HelminthProtozoaFungi_AssociationsFlatFile.csv")
bacteria <- read_csv("/Users/dcsuh/Desktop/clover/CLOVER_1.0_Bacteria_AssociationsFlatFile.csv")
```
...
Parasite Summary
```{r}
ggplot(pathogens, aes(PathogenType)) + geom_bar()
pathogens %>% filter(PathogenType=="bacteria/rickettsia") %>% ggplot(.,aes(PathogenClass)) +geom_bar()+theme(axis.text.x = element_text(angle=45,vjust=0.5))
pathogens %>% filter(PathogenType=="virus") %>% ggplot(.,aes(PathogenClass)) +geom_bar()+theme(axis.text.x = element_text(angle=45,vjust=0.6))
```

Host Summary
```{r}
ggplot(pathogens, aes(HostClass)) + geom_bar()
pathogens %>% filter(HostClass=="mammalia") %>% ggplot(.,aes(HostOrder)) + geom_bar() + theme(axis.text.x = element_text(angle=70, vjust = 0.5))
