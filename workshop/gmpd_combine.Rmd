...
select_out <- function(df) {
  dplyr::select(df, -c(HostCorrectedName, order:phylacine_binomial, adult_forearm_length_mm, biogeographical_realm))
}
```
...
gmpd_panth_helm %<>% mutate(foraging_stratum = as.factor(foraging_stratum))
gmpd_panth_helm %<>% mutate(island_endemicity = as.factor(island_endemicity))
# gmpd_panth_helm %<>% mutate(biogeographical_realm = as.factor(biogeographical_realm))
...
gmpd_panth_arth %<>% mutate(foraging_stratum = as.factor(foraging_stratum))
gmpd_panth_arth %<>% mutate(island_endemicity = as.factor(island_endemicity))
# gmpd_panth_arth %<>% mutate(biogeographical_realm = as.factor(biogeographical_realm))
...
gmpd_panth_virus %<>% mutate(foraging_stratum = as.factor(foraging_stratum))
gmpd_panth_virus %<>% mutate(island_endemicity = as.factor(island_endemicity))
# gmpd_panth_virus %<>% mutate(biogeographical_realm = as.factor(biogeographical_realm))
...
gmpd_panth_bact %<>% mutate(foraging_stratum = as.factor(foraging_stratum))
gmpd_panth_bact %<>% mutate(island_endemicity = as.factor(island_endemicity))
# gmpd_panth_bact %<>% mutate(biogeographical_realm = as.factor(biogeographical_realm))
...
gmpd_panth_proto %<>% mutate(foraging_stratum = as.factor(foraging_stratum))
gmpd_panth_proto %<>% mutate(island_endemicity = as.factor(island_endemicity))
# gmpd_panth_proto %<>% mutate(biogeographical_realm = as.factor(biogeographical_realm))
...
gmpd_panth_macro %<>% mutate(foraging_stratum = as.factor(foraging_stratum))
gmpd_panth_macro %<>% mutate(island_endemicity = as.factor(island_endemicity))
# gmpd_panth_macro %<>% mutate(biogeographical_realm = as.factor(biogeographical_realm))
...
gmpd_panth_micro %<>% mutate(foraging_stratum = as.factor(foraging_stratum))
gmpd_panth_micro %<>% mutate(island_endemicity = as.factor(island_endemicity))
# gmpd_panth_micro %<>% mutate(biogeographical_realm = as.factor(biogeographical_realm))
...
```{r}
get_pred <- function(model, test){
  gbm.predict.grids(model, test, want.grids=F, sp.name = "preds")
}
```

```{r}
full_pred <- get_pred(brt1_elith_full, gmpd_panth_test)
helm_pred <- get_pred(brt2_elith_helm, gmpd_panth_helm_test)
virus_pred <- get_pred(brt3_elith_virus, gmpd_panth_virus_test)
arth_pred <- get_pred(brt4_elith_arth, gmpd_panth_arth_test)
bact_pred <- get_pred(brt5_elith_bact, gmpd_panth_bact_test)
proto_pred <- get_pred(brt6_elith_proto, gmpd_panth_proto_test)
macro_pred <- get_pred(brt7_elith_macro, gmpd_panth_macro_test)
micro_pred <- get_pred(brt8_elith_micro, gmpd_panth_micro_test)
```
...
```{r}
full_test <- tibble(actual = gmpd_panth_test$psr,
                    pred = gbm.predict.grids(brt1_elith_full, gmpd_panth_test, want.grids=F, sp.name = "preds"))
full_test %<>% mutate(diff = actual-pred)
ggplot(full_test, aes(x=actual, y=pred)) + geom_point() + geom_abline(slope=1)
ggplot(full_test, aes(x=actual, y=diff)) + geom_point() + geom_hline(yintercept=0)
ggplot(full_test, aes(x=actual, y=abs(diff))) + geom_point() + geom_hline(yintercept=0)
```

```{r}
test_psr <- gmpd_panth_helm_test$psr
model <- brt2_elith_helm
test_data <- gmpd_panth_helm_test

full_test <- tibble(actual = test_psr,
                    pred = gbm.predict.grids(model, test_data, want.grids=F, sp.name = "preds"))
full_test %<>% mutate(diff = actual-pred)
ggplot(full_test, aes(x=actual, y=pred)) + geom_point() + geom_abline(slope=1)
ggplot(full_test, aes(x=actual, y=diff)) + geom_point() + geom_hline(yintercept=0)
ggplot(full_test, aes(x=actual, y=abs(diff))) + geom_point() + geom_hline(yintercept=0)
...
```{r}
test_psr <- gmpd_panth_virus_test$psr
model <- brt3_elith_virus
test_data <- gmpd_panth_virus_test
...
```{r}
test_psr <- gmpd_panth_arth_test$psr
model <- brt4_elith_arth
test_data <- gmpd_panth_arth_test
...
```{r}
test_psr <- gmpd_panth_bact_test$psr
model <- brt5_elith_bact
test_data <- gmpd_panth_bact_test
...
```{r}
test_psr <- gmpd_panth_proto_test$psr
model <- brt6_elith_proto
test_data <- gmpd_panth_proto_test
...
```{r}
test_psr <- gmpd_panth_macro_test$psr
model <- brt7_elith_macro
test_data <- gmpd_panth_macro_test
...
```{r}
test_psr <- gmpd_panth_micro_test$psr
model <- brt8_elith_micro
test_data <- gmpd_panth_micro_test
...
```{r}
brt3_virus_test_pred <- virus_pred
