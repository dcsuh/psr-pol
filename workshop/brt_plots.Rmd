...
```{r plot by type}
brt_ri %>% filter(par_type %in% c("arthropod", "helminth", "bacteria", "protozoa", "virus")) %>%
filter(ranking < 11) %>%
...
brt2_plots_max_long <- make_plots(brt2_elith_helm, "MaxLongevity_m")
brt3_plots_max_long <- make_plots(brt3_elith_virus, "MaxLongevity_m")
```



```{r max longevity full plot}
full_max_long <- ggdraw()+
  draw_plot(brt1_plots_max_long$histo, x=0.05, scale = 0.95) +
  draw_plot(brt1_plots_max_long$pdp, x=-0.03, scale = 0.975, y = 0.05)
full_max_long
ggsave(here("figures/pdp/full_max_long.png"), full_max_long)
```

```{r helminth max long}
helm_max_long <- ggdraw()+
  draw_plot(brt2_plots_max_long$histo, x=0.03, scale = 0.925) +
  draw_plot(brt2_plots_max_long$pdp, x=-0.0425, scale = 0.975, y = 0.065)
helm_max_long
ggsave(here("figures/pdp/helm_max_long.png"), helm_max_long)

```


```{r virus max long}
virus_max_long <- ggdraw()+
  draw_plot(brt3_plots_max_long$histo, x=0.03, scale = 0.95) +
  draw_plot(brt3_plots_max_long$pdp, x=-0.03, scale = 0.975, y = 0.055)
virus_max_long
ggsave(here("figures/pdp/virus_max_long.png"), virus_max_long)

```


```{r ref count pdp}
brt1_ref_count_plot <- make_plots(brt1_elith_full, "ref_count", y_fixed = T, y_axis = c(1,4))
brt2_ref_count_plot <- make_plots(brt2_elith_helm, "ref_count", y_fixed = T, y_axis = c(1,4))
brt3_ref_count_plot <- make_plots(brt3_elith_virus, "ref_count", y_fixed = T, y_axis = c(1,4))
brt4_ref_count_plot <- make_plots(brt4_elith_arth, "ref_count", y_fixed = T, y_axis = c(1,4))
brt5_ref_count_plot <- make_plots(brt5_elith_bact, "ref_count", y_fixed = T, y_axis = c(1,4))
brt6_ref_count_plot <- make_plots(brt6_elith_proto, "ref_count", y_fixed = T, y_axis = c(1,4))
brt7_ref_count_plot <- make_plots(brt7_elith_macro, "ref_count", y_fixed = T, y_axis = c(1,4))
brt8_ref_count_plot <- make_plots(brt8_elith_micro, "ref_count", y_fixed = T, y_axis = c(1,4))
```

```{r}
ggdraw()+
  draw_plot(brt1_ref_count_plot$histo, x=0.03, scale = 0.95) +
  draw_plot(brt1_ref_count_plot$pdp, x=-0.03, scale = 0.975, y = 0.055) +
  draw_plot(brt2_ref_count_plot$pdp, x=-0.03, scale = 0.975, y = 0.055) +
  draw_plot(brt3_ref_count_plot$pdp, x=-0.03, scale = 0.975, y = 0.055) +
  draw_plot(brt4_ref_count_plot$pdp, x=-0.03, scale = 0.975, y = 0.055) +
  draw_plot(brt5_ref_count_plot$pdp, x=-0.03, scale = 0.975, y = 0.055) +
  draw_plot(brt6_ref_count_plot$pdp, x=-0.03, scale = 0.975, y = 0.055)
```
