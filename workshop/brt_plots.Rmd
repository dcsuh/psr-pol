...
####### 1. coral area
inv<-pdp::partial(my.gbm,n.trees=best.iter,"coral.area",prob=T)

inv <- pdp::partial(brt1_elith_full, n.trees=brt1_elith_full$n.trees, "MaxLongevity_m", prob = T, train = brt1_elith_full$gbm.call$dataframe)
```
...
na.omit() %>%
ggplot() +
geom_line(aes(x = MaxLongevity_m, y = yhat),color="black",size=1) +
xlim(0,750)+
ylab(" ") +
theme_minimal() +
...
plot2 <- gmpd_panth_train %>%
dplyr::select(MaxLongevity_m) %>%
na.omit() %>%
ggplot(aes(MaxLongevity_m)) +
geom_histogram(breaks = seq(0,750, by = 10), fill="gray") +
...
```{r pdp}
p1 <- add_sub(plot1," ")
p2 <- add_sub(plot2,"Max Longevity",size=12)
p.max_long<-ggdraw() +
  draw_plot(p2,x=0.05,scale=0.95) +
  draw_plot(p1,x=-0.05,scale=0.975,y=0.05)
