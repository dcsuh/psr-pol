...
healy_axis_host <- healy_axis %>% filter(taxa_name == "Mammalia") %>% dplyr::select(Host) %>% distinct()
```
...
```{r}
healy_axis_psr_avg %>% filter(psr<10000) %>% ggplot(aes(x=gen_time, y=psr)) + geom_point()
healy_axis_psr_avg %>% filter(psr<10000) %>% ggplot(aes(x=gen_time, y=psr_h)) + geom_point()
healy_axis_psr_avg %>% filter(psr<10000) %>% ggplot(aes(x=gen_time, y=psr_mi)) + geom_point()
