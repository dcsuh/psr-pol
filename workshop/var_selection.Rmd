...
env_panth <- panth %>% select(env_vars)
anthro_panth <- panth %>% select(anthro_vars)
metabolic_panth <- panth %>% select(metabolic_vars)
morph_panth <- panth %>% select(morph_vars)
behavior_panth <- panth %>% select(behavior_vars)
```
...
```{r}
panth %<>% dplyr::select(!c(Precip_Mean_mm, Temp_Mean_degC, AET_Mean_mm, PET_Mean_mm)) # remove environmental variables
...
panth %<>% dplyr::select(!c(AdultBodyMass_g, AdultForearmLen_mm, AdultHeadBodyLen_mm, NeonateBodyMass_g,NeonateHeadBodyLen_mm, WeaningBodyMass_g, WeaningHeadBodyLen_mm)) # remove all morphology variables except for the extrapolated variables and Teat number
```
