...
panth1$ref_count <- str_count(panth1$References, ";")
```
...
```{r}
gmpd_VSR <- gmpd %>% filter(ParType %in% c("Virus", "Bacteria", "Protozoa")) %>% dplyr::select(HostCorrectedName,ParasiteCorrectedName) %>% distinct() %>% 
...
Let's deal with all the -999 and the references column and the numbers and weird stuff in the column names!!
```{r}
gmpd_panth_v[gmpd_panth_v == -999.00] <- NA
...
```{r}
gmpd_HSR <- gmpd %>% filter(ParType %in% c("Helminth", "Arthropod")) %>% dplyr::select(HostCorrectedName,ParasiteCorrectedName) %>% distinct() %>% 
...
Let's deal with all the -999 and the references column and the numbers and weird stuff in the column names!!
```{r}
gmpd_panth_h[gmpd_panth_h == -999.00] <- NA
...
p.maxLong  + labs(title = "full model")
```

```{r microparasite model results}
...
p.maxLong<-ggdraw() +
  draw_plot(p2,x=0.05,scale=0.95) +
  draw_plot(p1,x=-0.04,scale=0.975, y = 0.05)
p.maxLong + labs(title = "microparasite model")
```

```{r macroparasite model results}
...
p.maxLong<-ggdraw() +
  draw_plot(p2,x=0.05,scale=0.95) +
  draw_plot(p1,x=-0.04,scale=0.975,y=0.05)
p.maxLong + labs(title = "macroparasite model")
```
