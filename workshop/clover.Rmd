...
```{r}
eid <- pathogens %>% filter(Database == "EID2")
eid %>% ggplot(., aes(x=DetectionMethodOriginal)) + geom_histogram(stat = "count")
```

Hmm NCBI nucleotide seems like genbank while cargocarrier may just be the text mined ones.

```{r}
eid %>% filter(DetectionMethodOriginal == "NCBI Nucleotide") %>% ggplot(., aes(x=PathogenType)) + geom_histogram(stat = "count")
eid %>% filter(DetectionMethodOriginal == "NCBI Nucleotide") %>% ggplot(., aes(x=HostClass)) + geom_histogram(stat = "count")
eid %>% filter(DetectionMethodOriginal == "NCBI Nucleotide") %>% filter(HostClass == "mammalia") %>% ggplot(., aes(x=HostOrder)) + geom_histogram(stat = "count") + theme(axis.text.x = element_text(angle = 70, hjust = 1))
```
