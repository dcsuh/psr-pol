...
```{r}
hosts_intersect <- readRDS(here("data/hosts_intersect"))
```
...
```
```{r}
panth_subset <- panth1 %>% filter(Binomial %in% hosts_intersect)

panth_subset_missing <- makeMissing(panth_subset)

plotMissing(panth_subset_missing)
```
...
```{r}
combine_missing <- makeMissing(combine)

plotMissing(combine_missing)
```

```{r}
combine_cols <- tibble(df = "combine",
...
```{r}
all_cols <- rbind(amniote_cols, animal_cols, elton_cols, hormone_cols, panth_cols, combine_cols)
...
panth_subset_names <- tolower(unique(panth_subset$Binomial))
...
#              output=T)
# 
# venn.diagram(x = list(amn_mamm_names, elton_names, panth_names, clover_names),
#              category.names = c("amniote", "elton", "pantheria", "clover"),
#              filename = "hosts_clover_overlap.png",
#              output=T)
# 
# venn.diagram(x = list(panth_subset_names, panth_names),
#              category.names = c("gmpd_subset", "PanTHERIA"),
#              filename = here("workshop/venn_diagrams/gmpd_panth_subset_overlap.png"),
#              output=T)

```

Amniote, Elton, and Panth share 336 hosts with both gmpd and clover and about 660 with clover. Amniote, Elton, and Panth share 4940 with each other. 997 
