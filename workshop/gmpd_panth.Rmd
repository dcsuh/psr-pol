...
gmpd_panth_virus %<>% filter(MSW_Binomial %in% gmpd_panth_helm_hosts) #choose host species from macro that are also in micro
gmpd_panth_virus_hosts <- gmpd_panth_virus$MSW_Binomial #get remaining host species from micro database
gmpd_panth_helm %<>% filter(MSW_Binomial %in% gmpd_panth_virus_hosts) #make sure these are also in macro database
...
test <- gmpd_panth_ath %>% mutate(row_num = rownames(.)) %>% filter(!row_num %in% train) %>% dplyr::select(row_num)
test <- as.integer(test$row_num)
...
get_mse <- function(model, test) {
  ...
  mean((pred - test$psr)^2)
  sqrt(mean((pred-test$psr)^2))
}
```
