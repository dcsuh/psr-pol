...
gmpd_panth_macro %<>% filter(MSW_Binomial %in% intersect_hosts)
gmpd_panth_micro %<>% filter(MSW_Binomial %in% intersect_hosts)
```
...
train <- sample(1:nrow(gmpd_panth_micro), nrow(gmpd_panth_micro) * 0.8)
test <- gmpd_panth_micro %>% mutate(row_num = rownames(.)) %>% filter(!row_num %in% train) %>% dplyr::select(row_num)
test <- as.integer(test$row_num)
